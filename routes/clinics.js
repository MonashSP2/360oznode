...
router.get('/', (req, res, next) => {
    clinic.find().limit(10).exec().then(docs =>{
        console.log(docs);
...
router.get('/:xLowLeft/:yLowLeft/:xHighRight/:yHighRight', (req, res, next) => {
    ...
    clinic.find({latitude:{$gt:xLowLeft,$lt:xHighRight},longitude:{$gt:yLowLeft,$lt:yHighRight}}).sort( { votes: -1 } ).limit(50).exec().then(docs =>{
        console.log(docs);
